...
All new code will be in `src/mahindrabot/core/`:

- `src/mahindrabot/core/__init__.py` - Public API exports
- `src/mahindrabot/core/models.py` - Pydantic models (Skill, Intent, IntentType enum)
- `src/mahindrabot/core/toolkit.py` - AgentToolKit implementation
- `src/mahindrabot/core/skills.py` - Skill definitions
- `src/mahindrabot/core/intents.py` - Intent classification
- `src/mahindrabot/core/agent.py` - Core agent flow
- `src/mahindrabot/core/state.py` - State management
...
class Intent(BaseModel):
    ...
    intent_name: IntentType  # Enum value
...
4. **search_faq** - Wraps `faq_service.search()`
   ...
   - **Special handling**: If no relevant FAQs found (empty results or low scores), return a message: "I couldn't find any relevant information in our FAQ database. Please contact customer support for assistance."
...
5. **book_ride** - Initiates test drive booking
   ...
   - Sends Slack message with OTP using `slack.send_message()`
...
- Uses `get_llm_structured_response()` from `llm_service.core`
...
**Key change:** Only use the last user message, not the last 5 messages.
...
def classify_intent(messages: list[MessageType], llm_config: LLMConfig) -> Intent:
    ...
    if not user_messages:
        # Default to general_qna if no user messages
        return Intent(
...
Flow (updated to match mermaid diagram):
1. **Agent is already initialized at the start**
...
7. Update agent with new system message and tools
8. Stream `AgentResponse` back to caller
...
Remember: The user cannot see tool calls or results. Present information naturally as if you knew it directly.
```

**Implementation:**
```python
from collections.abc import Generator
from src.mahindrabot.services.llm_service import (
...
def run_mahindra_bot(
    ...
    """
    Main bot flow: classify intent, load skill, run agent with appropriate tools.
    ...
    # Classify intent based on last message
    ...
    # Filter tools to only those relevant for this skill
    all_tools = toolkit.get_tools()
    relevant_tools = [t for t in all_tools if t.name in skill.relevant_tools]
...
1. **Car Service**: Use existing `CarService` with its error handling
2. **FAQ Service**: Use existing `FAQService` for semantic search
3. **LLM Service**: Use `StreamingChatWithTools`, `get_llm_structured_response`
4. **Slack Service**: Use `send_message()` for OTP notifications
5. **Models**: Use `CarDetail`, `QNAResult`
...
1. `src/mahindrabot/core/__init__.py`
2. `src/mahindrabot/core/models.py`
3. `src/mahindrabot/core/state.py`
4. `src/mahindrabot/core/toolkit.py`
5. `src/mahindrabot/core/skills.py`
6. `src/mahindrabot/core/intents.py`
7. `src/mahindrabot/core/agent.py`

## Implementation Checklist

- [ ] Create core/models.py with IntentType enum, Intent, and Skill models
- [ ] Implement core/state.py with StateManager for OTP management
- [ ] Build core/toolkit.py with AgentToolKit class and all 6 tool implementations using ToolKit.register()
- [ ] Define skills in core/skills.py with instructions and tool mappings (search_faq only in general_qna)
- [ ] Implement core/intents.py with intent classification focusing on last message only
- [ ] Create core/agent.py with run_mahindra_bot function implementing main flow
- [ ] Setup core/__init__.py with clean public API exports
- [ ] Create demo script showcasing all 4 intent flows with example conversations

## Next Steps After Implementation

1. Create comprehensive demo script with examples for all 4 intents
2. Add logging for intent classification and tool calls
3. Add unit tests for each component
4. Create integration tests for full conversation flows
